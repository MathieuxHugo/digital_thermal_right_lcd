...
class LEDDisplayUI:
    ...
    def create_phantom_spirit_layout(self):
        ...
        watts_frame = self.create_watts_frame(led_frame)
        ...
        while True:
            try:
                ...
                if METRICS_AVAILABLE:
                    ...
                    metrics = {'cpu_usage': 45, 'gpu_usage': 30, 'cpu_temp': 55, 
                              'gpu_temp': 60, 'cpu_watts': 50, 'gpu_watts': 80}
                
                # Processa cores (mesmo método do controller)
                color_key = self.config.get("color_mode", "usage")
                colors = self.get_config_colors(self.config, key=color_key, metrics=metrics)
                
                # Aplica cores
                for index in range(len(colors)):
                    self.set_ui_color(index, color="#" + colors[index])
    ...
    def get_config_colors(self, config, key="usage", metrics=None):
        ...
        if not conf_colors:
            conf_colors = ["ffffff"] * NUMBER_OF_LEDS
        elif len(conf_colors) != NUMBER_OF_LEDS:
            if len(conf_colors) == 1:
                conf_colors = [conf_colors[0]] * NUMBER_OF_LEDS
            else:
                repeated = []
        ...
        for i, color in enumerate(conf_colors):
            if color.lower() == "random":
                ...
                # Processa gradiente por métrica
                parts = color.split(';')
                metric = parts[0]
                stops = []
                for stop in parts[1:]:
                ...
                if metric == "usage":
                    metric_value = metrics.get("cpu_usage", 0)
                ...
                if metric_value <= stops[0]['value']:
                    colors.append(stops[0]['color'])
                elif metric_value >= stops[-1]['value']:
                    colors.append(stops[-1]['color'])
                else:
                    for j in range(len(stops) - 1):
                        if stops[j]['value'] <= metric_value < stops[j+1]['value']:
                            factor = (metric_value - stops[j]['value']) / (stops[j+1]['value'] - stops[j]['value'])
                            colors.append(interpolate_color(stops[j]['color'], stops[j+1]['color'], factor))
                            break
            elif "-" in color:
                # Gradiente temporal
                colors_list = color.split("-")
                if len(colors_list) >= 2:
                    time_in_cycle = (time.time() - self.start_time) % self.cycle_duration
                    segment_duration = self.cycle_duration / (len(colors_list) - 1)
                    segment_index = min(int(time_in_cycle / segment_duration), len(colors_list) - 2)
                    factor = (time_in_cycle % segment_duration) / segment_duration
                    colors.append(interpolate_color(colors_list[segment_index], colors_list[segment_index + 1], factor))
                else:
    ...
    def create_watts_frame(self, root):
        frame = ttk.LabelFrame(root, text="Consumo (Watts)", padding=(10, 10))
        frame.grid(row=2, column=1, padx=10, pady=10) # Posicionado ao lado da Temperatura
        
        # LED do símbolo 'W'
        self.create_label(frame, "watts_w_led", "W", 0, 0)
        
        # Frame para os 3 dígitos (100s, 10s, 1s)
        digit_frame = ttk.Frame(frame, padding=(5, 5))
        digit_frame.grid(row=1, column=0, padx=5, pady=5)
        
        # Centenas
        self.create_segmented_digit_layout(digit_frame, "watts_100s_digit", number_of_digits=1, index=0)
        
        # Dezenas
        digit_frame2 = ttk.Frame(frame, padding=(5, 5))
        digit_frame2.grid(row=1, column=1, padx=5, pady=5)
        self.create_segmented_digit_layout(digit_frame2, "watts_10s_digit", number_of_digits=1, index=0)
        
        # Unidades
        digit_frame3 = ttk.Frame(frame, padding=(5, 5))
        digit_frame3.grid(row=1, column=2, padx=5, pady=5)
        self.create_segmented_digit_layout(digit_frame3, "watts_1s_digit", number_of_digits=1, index=0)
    ...
    def create_watts_frame(self, root):
        frame = ttk.LabelFrame(root, text="Power (Watts)", padding=(10, 10))
        frame.grid(row=1, column=2, padx=10, pady=10)
        
        # W LED
        self.create_label(frame, "watts_w_led", "W", 0, 0)
        
        # 3-digit watts display  
        digit_frame = ttk.Frame(frame, padding=(5, 5))
        digit_frame.grid(row=1, column=0, padx=5, pady=5)
        self.create_segmented_digit_layout(digit_frame, "watts_100s_digit", number_of_digits=1, index=0)
    ...
    def create_temp_frame(self, root):
        ...
        digit_frame = ttk.Frame(frame, padding=(5, 5))
        digit_frame.grid(row=1, column=0, columnspan=1, padx=5, pady=5)
    ...
    def create_color_mode(self, root, row=0, column=1):
        color_mode_frame = ttk.LabelFrame(root, text="Color mode:", padding=(10, 10))
        color_mode_frame.grid(row=row, column=column, pady=10)        
        self.color_mode = tk.StringVar(value=self.config.get("color_mode", "usage"))
        group_dropdown = ttk.Combobox(color_mode_frame, textvariable=self.color_mode, state="readonly")
        group_dropdown["values"] = ["usage", "time", "metrics"]  # ADICIONAR "usage"
        group_dropdown.grid(row=0, column=0, padx=5, pady=5)
        group_dropdown.bind("<<ComboboxSelected>>", lambda event: self.change_color_mode())
    
    def change_color_mode(self):  # ADICIONAR ESTE MÉTODO
